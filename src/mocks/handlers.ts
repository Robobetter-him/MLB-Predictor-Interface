...
function generateGamesForDate(date: string) {
...
export const handlers = [
  // Get teams
  http.get('/api/teams', () => {
    return HttpResponse.json(Object.keys(mlbTeams).sort());
  }),
  
  // Get team details
  http.get('/api/teamDetails', () => {
    return HttpResponse.json(mlbTeams);
  }),
  ...
  http.post('/api/predict', async ({ request }) => {
    ...
    if (!homeTeam || !awayTeam || homeTeam === awayTeam) {
      return new HttpResponse(
        JSON.stringify({ error: "Invalid team selection" }),
        { status: 400 }
      );
    }
    ...
    if (prediction) {
      ...
      return new HttpResponse(
        JSON.stringify({ error: "Could not generate prediction" }),
        { status: 400 }
      );
    }
  }),
  ...
  http.get('/.netlify/functions/teams', () => {
    return HttpResponse.json(Object.keys(mlbTeams).sort());
  }),
  
  http.get('/.netlify/functions/teamDetails', () => {
    return HttpResponse.json(mlbTeams);
  }),
  
  http.get('/.netlify/functions/games', ({ request }) => {
    const url = new URL(request.url);
    const date = url.searchParams.get('date') || format(new Date(), 'yyyy-MM-dd');
    const games = generateGamesForDate(date);
    return HttpResponse.json(games);
  ...
  http.get('/.netlify/functions/historical', () => {
    const historicalData = generateHistoricalData();
    return HttpResponse.json(historicalData);
  }),
  
  http.post('/.netlify/functions/predict', async ({ request }) => {
    const body = await request.json();
    const { homeTeam, awayTeam } = body;
  ...
  // RapidAPI Baseball handlers
  http.get('/.netlify/functions/rapidApiBaseball/teams', () => {
    return HttpResponse.json(Object.keys(mlbTeams).sort());
  }),
  
  http.get('/.netlify/functions/rapidApiBaseball/games', ({ request }) => {
    const url = new URL(request.url);
    const date = url.searchParams.get('date') || format(new Date(), 'yyyy-MM-dd');
    const games = generateGamesForDate(date);
    return HttpResponse.json(games);
  ...
  http.post('/.netlify/functions/rapidApiBaseball/predict', async ({ request }) => {
    const body = await request.json();
    const { homeTeam, awayTeam } = body;
